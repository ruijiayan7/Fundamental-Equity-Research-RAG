...
def generate_recommended_questions(user_question, retrieved_content=None, session_id=None):
   ...
   # 构造优化后的提示词
    ...
    prompt = f"""
    You are an intelligent assistant. Based on the user's question, generate 3 relevant follow-up questions to help the user explore the topic more deeply.

User Question: {user_question}
{context_info}

Requirements:
1. The generated questions should be related to the user's question but explore it from different angles
2. Questions should be specific and valuable, guiding the user toward more useful information
3. If document context is provided, generate questions relevant to the document's theme
4. Return JSON format containing a recommended_questions array
...
def generate_session_name(user_question):
    prompt = f"""
    Based on the user's question below, generate a concise and representative session name:
    User Question: {user_question}

    Requirements:
    1. The session name should be concise and clearly capture the main topic of the user's question.
    2. Return a JSON object containing a single field "session_name" with the generated session name as its value.

    Output format example:
    {{
        "session_name": "Session Name Content"
    }}

    Please strictly follow the format above and return only the JSON object.
    """
...
def get_chat_completion(session_id, question, retrieved_content, user_id):
    ...
    prompt = f"""
You are a professional intelligent assistant, skilled at answering user questions based on provided reference materials. Please follow these principles:

**Response Requirements:**
1. Prioritize answering based on the reference content to ensure accuracy and reliability
2. Every piece of information in your response must be cited with its source, using the format: ##citation_number$$. For example: ##1$$ indicates the content is cited from reference #1.
3. If the reference content is insufficient to fully answer the question, you may supplement with general knowledge, but clearly distinguish it from cited content
4. Keep your response well-structured and naturally fluent
5. If no relevant reference content exists, honestly state so and provide general advice
...
Please provide a professional and accurate response based on the information above.
    """
